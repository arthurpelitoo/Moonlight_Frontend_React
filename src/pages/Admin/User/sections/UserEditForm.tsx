...
import { isUserTypeValid } from "../../../../utils/Validation/dataRules/userType";
import { isCPFValid } from "../../../../utils/Validation/dataRules/cpf";
import { isEmailValid } from "../../../../utils/Validation/dataRules/email";
import { useUserEditForm } from "../../../../hooks/validation/Admin/useUserEditForm";
...
export function UserEditForm({user} : UserEditFormProps){
    const {fields, selectOptions, setField, setCpf, showErrors, toggleShowConfirm, toggleShowPassword, ui, handleBlur, handleSubmit} = useUserEditForm({
        name: user?.name ?? "",
        email: user.email ?? "",
        cpf: user?.cpf ?? "",
    ...
    if (ui.submitted && !ui.apiError && ui.success) {
        return (
            <div className="flex flex-col items-center gap-6 py-8 text-center animate-fade-in">
                ...
                <h3 className="text-white text-lg font-light tracking-wider">Conta editada!</h3>
            </div>
        );
    }
    ...
    return (
        <div className="flex flex-col gap-3 w-full">
            ...
            <div className="max-lg:flex max-lg:flex-col lg:grid lg:grid-cols-3 gap-4">
                <div>
                    <InputFieldForm
                        ...
                        icon={<LockKeyIcon size={18} />} placeholder="Reescreva a senha ou coloque uma nova"
                        rightElement={
            ...
            >
                {ui.loading ? <LoadingDots /> : <>Editar Usuario <ArrowRightIcon size={16} weight="bold" /></>}
            </Button>
        </div>
    )
}